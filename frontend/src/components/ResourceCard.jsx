...
function ResourceCard({ resource, isMobile = false }) {
  ...
  if (isMobile) {
    ...
    return (
      <div 
        ...
        className="flex items-center gap-4 p-4 bg-amber-50 hover:bg-amber-100 rounded-lg border border-amber-200 relative cursor-pointer transition-all hover:shadow-md"
      >
        <img
          src={resource.image}
          alt={resource.title}
  ...
  return (
    <div 
      onClick={handleCardClick}
      className="flex flex-col bg-amber-50 hover:bg-amber-100 rounded-lg border border-amber-200 overflow-hidden shadow-sm hover:shadow-md relative cursor-pointer transition-all transform hover:-translate-y-1"
    >
